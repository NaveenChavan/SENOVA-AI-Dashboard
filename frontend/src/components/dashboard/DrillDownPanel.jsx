 ...
 *
 * Motion note: the open/close transition is owned by AnimatePresence, which
 * needs the exiting element to stay mounted until its exit animation finishes.
 * The two effects below are unchanged from the pre-motion version — they still
 * key off `selection` becoming truthy/falsy — but the component itself never
 * early-returns null; instead `selection` gates the AnimatePresence child, so
 * Motion (not the effects) controls the actual unmount timing.
 */
