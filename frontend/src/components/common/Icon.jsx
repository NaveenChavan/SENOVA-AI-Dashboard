/**
 * Inline SVG icon set (Heroicons-style outline paths).
 *
 * Emoji are explicitly ruled out as icons by the design guidelines: they render
 * differently per platform, can't inherit colour, and read as literal text to
 * screen readers. Bundling only the ~12 paths this dashboard needs avoids a
 * whole icon dependency while keeping every glyph on-brand and themeable via
 * ``currentColor``.
 *
 * Decorative icons are ``aria-hidden`` by default; pass a ``label`` when the
 * icon is the only thing conveying meaning, and it becomes an accessible image.
 */
...
const PATHS = {
  ...
  // Trends
  trendUp: 'M2.25 18L9 11.25l4.306 4.307A11.95 11.95 0 0119.8 9.75M21.75 9.75V4.5m0 5.25h-5.25',
  trendDown: 'M2.25 6L9 12.75l4.306-4.307A11.95 11.95 0 0119.8 14.25M21.75 14.25V19.5m0-5.25h-5.25',
  // Objects
...
export default function Icon({ name, className = 'w-5 h-5', label, strokeWidth = 1.6, ...rest }) {
  ...
  return (
    <svg
      ...
      // Decorative unless the caller gives it a name to announce.
